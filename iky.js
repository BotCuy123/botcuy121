...
if (args.length < 2) return reply(`Penggunaan ${command} idgroup`)
        ...
        case 'owner':
        case 'creator':
               sendKontak(from, `6281337744739`, `Nanda Style`, 'Sibukk!!')
               await sleep(1000)
...
*NOMOR CREATOR*
▼▼▼▼▼`)
sendKontak(from, `6281337744739`, `${ownerName}`, 'Sibukk!!')
               await sleep(1000)
               break
               case 'spamcall':{
...
case 'lock':
 ikymemek = {
  "key": {
    "fromMe": false,
    participant: `6281337744739@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
  },
  "message": {
    "viewOnceMessage": {
      "message": {
...
🐟 ${prefix}wallgravity text1 text2
 `
sendButMessage(from, wibu, `created KIRIYUBOTZ`, [
          {
            buttonId: `!menu`,
            buttonText: {
...
`
        ...
        break;
			...
			case 'blur':
              ...
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'KIRIYUBOTZ', imageMessage: imageMsg,
...
`
 ...
 KIRIYU BOT☕︎`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 1
}
...
`
 ...
 KIRIYUBOTZ☕︎`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 1
}
...
`
 ...
 KIRIYUBOTZ☕︎`, imageMessage: imageMsg,
   ...
   case 'command':
               list = []
               listmenu = [`groupmenu`,`photoxy`,`makermenu`,`ephoto`,`randomimage`,`wibu2`,`storymenu`,`randomtext`,`islammenu`,`wibumenu`,`stickermenu`,`ownermenu`,`gamemenu`,`funmenu`,`downloadmenu`,`infomenu`,`othermenu`,`owner`,`gcmikel`,`sewabot`]
               listmenuu = [`Menu Group`,`Photo Oky`,`Image Maker`,`Ephoto Menu`,`Random Image`,`Movie&Story`,`RandomText`,`Islam Menu`,`Wibu Menu`,`Sticker Menu`,`Owner Command`,`Game Menu`,`For Fun Menu`,`Downloader`,`Info Menu`,`Menu Lainnya`,`OwnerBot`,`Official Group`,`Rent this Bot`,`Sewa Bot`]
               nombor = 1
               startnum = 0
...
break
           ...
           case 'liststicker':
            ...
            case 'done':
              if (!isGroup) return reply(mess.only.group)
              reply('Orderan Dari Buyer Ke KIRIYUBOTZ Telah Berhasil\nTerimakasih Telah Order Di KIRIYUBOTZ 😊\nJangan Lupa Order Kembali👋')
             break
			case 'getvideo':
...
*「 KIRIYUBOTZ 」*`
	    ...
	    // console.log(e)
            setTimeout( () => {
			ikyy.updatePresence(from, Presence.composing)
			reply(`*Terimakasih Telah Mendaftar Di KIRIYUBOTZ🐟*\n*Masuk Gc Bot Yuk ↓*\n*https://chat.whatsapp.com/FdxEeGx290X81uifrYsgZx*\n*Terimakasih*`)
		}, 2000)
        break
case 'sfire':
{
...
//------------------< Premium >-------------------
       ...
       case 'sewabot':
              gopeynya = 'https://i.ibb.co/nMQvfzm/Screenshot-2021-11-17-06-34-45-82.jpg'
...
//------------------< Ingfo Bot >-------------------
      ...
      case 'donate': 
       case 'donasi':
            ...
            buff = await getBuffer(gopeynya)
            teksnya = `
DONASI OM\n*DANA/GOPAY/PULSA : 081337744739*\n*SESUNGGUH NYA ORANG YANG MEMBERI LEBIH BAIK DARI MENERIMA ☝️* 
`
  buttons = [{buttonId: `${prefix}owner`,buttonText:{displayText: `Owner👑`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'© Kiriyu Botz', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              ikyy.relayWAMessage(prep)
              break
...
`
  buttons = [{buttonId: `${prefix}owner`,buttonText:{displayText: `Owner👑`},type:1},{buttonId: `${prefix}payment`,buttonText:{displayText: `PAYMENT`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'NOTE📑:HANYA DI Kiriyu', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              ikyy.relayWAMessage(prep)
              break
...
ikyy.sendMessage(_.jid, 
			{"contentText": `*「 KIRIYUBOTZ NEWS 」*\n\n${body.slice(4)}`,
			"footerText": 'KIRIYUBOTZ',
			"buttons": [
			{"buttonId": `#command`,
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'bugreport': 
              ...
              ikyy.sendMessage('6281337744739@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'jawaban':
       ...
       case 'request':
              ...
              ikyy.sendMessage('6281337744739@s.whatsapp.net',`*Request:* ${teks}`, text)
              break
case 'chatrava':
              if (args.length < 1) return reply(`Ketik ${prefix}CHAT [CHAT apa]`) 
              teks = args.join(' ')
              reply('CHAT NANDA DONE')
              ikyy.sendMessage('6281337744739@s.whatsapp.net',`*chat:* ${teks}`, text)
              break
       case 'readall':
